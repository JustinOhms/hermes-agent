...
### RD-22: Context injection for routing transitions

**Implemented 2026-06-12:** When any routing transition occurs (escalation, de-escalation, or manual upgrade/downgrade), the active model receives a context injection that explains the transition and provides relevant guidance. This prevents the new model from being "cold" — it knows why it was called and what the previous model was doing.

#### Injection types:

| Transition | Trigger | Injection content |
|------------|---------|-------------------|
| **Escalate** | Oversight ESCALATE | "↑ ESCALATION from {lower_model}: {reason}. You are now handling this turn. Review recent context and proceed." |
| **De-escalate** | Handback from upper | "↓ DE-ESCALATION from {upper_model}. Work can resume on {lower_model}. {handback_summary}" |
| **Upgrade** | User `@opus` / manual | "↑ UPGRADE requested by user. Switching from {lower_model} to {upper_model}. Proceed with current task." |
| **Downgrade** | User `@local` / manual | "↓ DOWNGRADE requested by user. Switching from {upper_model} to {lower_model}. Proceed with current task." |

#### Implementation:

The injection is a system message inserted into `messages` list right before the user's latest message. It uses `build_routing_transition_injection()` (analogous to `build_oversight_injection()`).

This pattern ensures:
- The model knows its new role without re-reading the entire history
- The user gets a clear explanation of what happened
- Context compression (RD-7) can piggyback on this injection
...
{
    "role": "system",
    "content": f"[ROUTING TRANSITION: {transition_type.upper()}]\n"
               f"Trigger: {trigger}\n"
               f"From: {from_model}\n"
               f"To: {to_model}\n"
               f"Reason: {reason}\n"
               f"\n{'Context: ' + context if context else ''}\n"
               f"\nYou are now active. Proceed with the current task."
}
```

#### Integration points:
- `conversation_loop.py` — call `build_routing_transition_injection()` after `switch_model()`
- `agent/routing/__init__.py` — inject on `execute_routing_swap()` success
- `tui_gateway/server.py` — emit routing status update to TUI

### RD-23: Handback prompt on de-escalation

**Implemented 2026-06-12:** When the upper model de-escalates (returns to lower model), it produces a **handback prompt** — a compressed context summary + guidance package. This is the inverse of escalation injection.

The handback:
1. Is generated by the upper model during its turn (if it decides to step down)
2. Is injected as a system message when the lower model resumes
3. Includes: active files, current task state, key decisions made, remaining tasks, guidance

This ensures the lower model can continue seamlessly without re-reading the upper model's entire response.

### RD-24: Manual routing override context

**Implemented 2026-06-12:** When the user manually triggers a routing transition via slash commands or special prefixes (`@opus`, `@local`), a context injection explains the manual trigger.

This prevents the model from assuming the switch was due to an error or complexity — it knows the user explicitly requested it. The injection includes the user's original intent if provided (e.g., `@opus please review my architecture`).
